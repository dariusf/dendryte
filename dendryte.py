...
class New(webapp2.RequestHandler):
    def get(self):
        ...
        if user:  # check if signed in
            ...
            item.title = "Untitled"
            # escape json before saving it to the database
            item.contents = urllib.quote("{\"nodes\":[]}", "")
...
class Save(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        # escape json before saving it to the database
        item.contents = urllib.quote(self.request.get('contents'), "")
        key = item.put();
